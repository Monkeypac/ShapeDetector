...
std::vector<std::vector<point>> b2Separator::Separate(std::vector<point>* pVerticesVec, int scale) {
    int i, n=pVerticesVec->size();
    std::vector<point> vec;
	std::vector<std::vector<point> > figsVec;

    
    for (i=0; i<n; i++) {
        vec.push_back(point(pVerticesVec->at(i).x*scale,pVerticesVec->at(i).y*scale));
    }
    
    calcShapes(vec, figsVec);
	int dx, dy;
    for (int k = 0; k < (int)figsVec.size(); k++){
		if (figsVec[k].size() < 3){
			figsVec.erase(figsVec.begin() + k);
			k--;
			continue;
		}
		dx = 0;
		dy = 0;
        for(int w = 0; w < (int)figsVec[k].size() - 1; w++){
			dx += std::abs(figsVec[k][w + 1].x - figsVec[k][w].x);
			dy += std::abs(figsVec[k][w + 1].y - figsVec[k][w].y);
		}
		if (flags & DEBUG) Log::debug() << dx << dy << (dx < 3 && dx > -3) << (dy < 3 && dy > -3);
		if ((dx < 3 && dx > -3) || (dy < 3 && dy > -3)){
			figsVec.erase(figsVec.begin() + k);
			k--;
			if (flags & DEBUG) Log::debug() << "delete !";
		}
	}
	return figsVec;
}
		 ...
		 * */        
int b2Separator::Validate(const std::vector<point> &verticesVec) {
...
void b2Separator::calcShapes(std::vector<point> &pVerticesVec, std::vector<std::vector<point> > &result) {
    std::vector<point> vec;
    int i, n, j,minLen;
    float d, t, dx, dy;
    int i1, i2, i3;
...
point* b2Separator::hitRay(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) {
    float t1 = x3-x1;
    float t2 = y3-y1;
    float t3 = x2-x1;
    float t4 = y2-y1;
...
point* b2Separator::hitSegment(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) {
    float t1 = x3-x1;
    float t2 = y3-y1;
    float t3 = x2-x1;
    float t4 = y2-y1;
